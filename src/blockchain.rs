...
impl TriangleState {
    ...
    /// If three children exist in the UTXO set that reference the provided
    /// `parent_hash`, and those children are effectively empty (have negligible
    /// effective value), attempt to merge them back into their parent. This
    /// can be used as a maintenance routine to coalesce tiny/empty fragments
    /// produced by subdivision.
    pub fn merge_parent_if_empty(&mut self, parent_hash: &Sha256Hash) -> Result<bool, ChainError> {
        // Collect child hashes that reference this parent_hash
        let children: Vec<(Sha256Hash, Triangle)> = self
            .utxo_set
            .iter()
            .filter_map(|(h, t)| {
                if let Some(ph) = t.parent_hash {
                    if &ph == parent_hash {
        ...
        // Extract only the triangle values for merging
        let tris: [Triangle; 3] = [children[0].1.clone(), children[1].1.clone(), children[2].1.clone()];

        // Attempt reconstruction
        if let Some(parent) = crate::geometry::Triangle::try_merge(&tris) {
            // Remove children from utxo_set and address_index
            for (h, t) in children.iter() {
                self.utxo_set.remove(h);
                if let Some(hashes) = self.address_index.get_mut(&t.owner) {
                    hashes.retain(|hh| hh != h);
                    if hashes.is_empty() {
                        self.address_index.remove(&t.owner);
                    }
                }
            }

            // Insert reconstructed parent
            let parent_hash_new = parent.hash();
            self.utxo_set.insert(parent_hash_new, parent.clone());
            self.address_index
                .entry(parent.owner.clone())
                .or_insert_with(Vec::new)
                .push(parent_hash_new);
    ...
    /// Run a maintenance pass to merge empty children back into reconstructed parents.
    /// Scans the UTXO set for any parents referenced by three children with negligible value.
    /// This is non-blocking and can be called periodically.
    pub fn run_maintenance_merge(&mut self) -> usize {
        let mut merge_count = 0;
        // Collect parent hashes that have children (to avoid modifying while iterating)
        let parent_hashes: Vec<Sha256Hash> = self
            .utxo_set
            .values()
            .filter_map(|t| t.parent_hash)
            .collect::<std::collections::HashSet<_>>()
            .into_iter()
            .collect();

        for parent_hash in parent_hashes {
            if let Ok(true) = self.merge_parent_if_empty(&parent_hash) {
                merge_count += 1;
            }
        }
        merge_count
...
mod tests {
    ...
    #[test]
    fn test_merge_empty_children_back_to_parent() {
        let mut state = TriangleState::new();
        let owner = "alice".to_string();

        // Create a parent triangle
        ...
        // Subdivide it into three children
        let children = parent.subdivide();

        // Insert children into state with near-zero effective values to simulate empty fragments
        for child in &children {
            let child_hash = child.hash();
            state.utxo_set.insert(child_hash, child.clone());
            state
                .address_index
                .entry(owner.clone())
                .or_insert_with(Vec::new)
        ...
        // Attempt merge
        let merge_result = state.merge_parent_if_empty(&parent_hash);
        assert!(merge_result.is_ok());
        assert!(merge_result.unwrap());

        // After merge, UTXO set should have exactly one entry (the reconstructed parent)
        assert_eq!(state.utxo_set.len(), 1);

        // The reconstructed parent should have the same total effective value
        let reconstructed = state.get_triangles_by_owner(&owner);
        assert_eq!(reconstructed.len(), 1);
        assert!(
            (reconstructed[0].effective_value() - parent.effective_value()).abs()
                < Coord::from_num(1e-6)
        );
    }

    #[test]
    fn test_run_maintenance_merge() {
        let mut state = TriangleState::new();
        let owner = "bob".to_string();

        // Create two parent triangles
        let parent1 = Triangle::new(
            Point::new(Coord::from_num(0), Coord::from_num(0)),
            Point::new(Coord::from_num(5), Coord::from_num(0)),
            Point::new(Coord::from_num(0), Coord::from_num(5)),
            None,
            owner.clone(),
        );
        let _parent1_hash = parent1.hash();

        let parent2 = Triangle::new(
            Point::new(Coord::from_num(10), Coord::from_num(0)),
            Point::new(Coord::from_num(15), Coord::from_num(0)),
            Point::new(Coord::from_num(10), Coord::from_num(5)),
            None,
            owner.clone(),
        );
        let _parent2_hash = parent2.hash();

        // Subdivide both parents
        let children1 = parent1.subdivide();
        let children2 = parent2.subdivide();

        // Insert all children
        for child in children1.iter().chain(children2.iter()) {
            let child_hash = child.hash();
            state.utxo_set.insert(child_hash, child.clone());
            state
                .address_index
        ...
        assert_eq!(state.utxo_set.len(), 6);

        // Run maintenance merge
        let merge_count = state.run_maintenance_merge();

        // Both parent sets should merge
        assert_eq!(merge_count, 2);
        assert_eq!(state.utxo_set.len(), 2);

        let remaining = state.get_triangles_by_owner(&owner);
